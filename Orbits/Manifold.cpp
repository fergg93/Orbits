...
void Manifold::Initialize( void )
{
  ...
  // Calculate radii from one object to another

  Vec2 r_a = A->position - B->position; 
  Vec2 r_b = B->position - A->position;  

	//Calculate magnitude of gravitational force
  Vec2 gravA = B->m*(1/r_b.LenSqr())*r_b;
  Vec2 gravB = A->m*(1/r_a.LenSqr())*r_a;

  //Apply gravitational correction
	A->ApplyImpulse (gravA,r_b);	
  B->ApplyImpulse (gravB,r_a);
